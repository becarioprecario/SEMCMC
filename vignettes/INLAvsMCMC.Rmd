...
In this example we compare the implementation of several Spatial Econometrics models using INLA and MCMC. In particular, the implementation of these methods with INLA is based on the `slm` latent class, whilst the implementation with MCMC has been done with `jags` and it is available in package `SEjags`.
...
parameter. This is done by using the eigenvalues of the adjacency matrix.
...
```
The precision matrix of the prior of the coefficients of the covariates in the latent effect
...
Q.beta = Diagonal(n = ncol(mmatrix), x = betaprec)
#Regression model with lagged covariates
Q.beta2 = Diagonal(n = ncol(mmatrix2), x = betaprec)
```
...
in the model that are not part of the `slm` latent effect. These
are the same as in the MCMC model.
```{r results = "hide"}
...
MCMC, the values of the random effects are computed by taking the difference
